...
def _load_environment(argv: Sequence[str] | None) -> Path:
    try:
        from dotenv import load_dotenv
    except ImportError as exc:  # pragma: no cover - depends on optional installation
        raise SystemExit("Install WebDAV dependencies: uv sync --extra webdav") from exc

    bootstrap = argparse.ArgumentParser(add_help=False)
    bootstrap.add_argument("--env-file", default=".env")
    args, _unknown = bootstrap.parse_known_args(argv)
    env_path = Path(args.env_file).expanduser()
    load_dotenv(env_path, override=False)
    return env_path


def _parser(*, env_file: Path | str = ".env") -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Expose AnyShare document libraries over WebDAV")
    parser.add_argument("--env-file", default=str(env_file), help="Load settings from this dotenv file")
...
def main(argv: Sequence[str] | None = None) -> int:
    env_file = _load_environment(argv)
    parser = _parser(env_file=env_file)
