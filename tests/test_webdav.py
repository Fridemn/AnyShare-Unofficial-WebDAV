...
from anyshare_unofficial.webdav.cli import _load_environment, _parser
...
class TestWebDAVEnvironment(unittest.TestCase):
    def test_dotenv_is_loaded_and_process_environment_wins(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            env_path = Path(temp_dir) / "gateway.env"
            env_path.write_text(
                "\n".join(
                    [
                        "ANYSHARE_BASE_URL=https://from-file.example",
                        "ANYSHARE_DAV_HOST=0.0.0.0",
                        "ANYSHARE_DAV_PORT=8088",
            ...
            with patch.dict(os.environ, {"ANYSHARE_DAV_PORT": "9090"}, clear=True):
                loaded_path = _load_environment(["--env-file", str(env_path)])
                args = _parser(env_file=loaded_path).parse_args(["--env-file", str(env_path)])

                self.assertEqual(loaded_path, env_path)
                self.assertEqual(args.base_url, "https://from-file.example")
                self.assertEqual(args.host, "0.0.0.0")
                self.assertEqual(args.port, 9090)
                self.assertTrue(args.readonly)
                self.assertEqual(args.cache_ttl, 12.5)

    def test_command_line_overrides_dotenv_booleans(self) -> None:
